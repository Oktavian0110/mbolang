...
<div class="container-fluid"> 
	<br>    
   	...
   	<a href="wisata_tambah.php" class="btn btn-outline-dark"><b>Input Wisata</b></a>
	...
	<table id="tabel-tampil" class="table table-bordered table-striped">
		...
		if($_SERVER['REQUEST_METHOD'] == "POST"){
			...
			if($pencarian != ""){
				$sql = "SELECT * FROM tbwisata WHERE nama LIKE '%$pencarian%'
				...
				$query = "SELECT * FROM tbwisata LIMIT $posisi, $batas";
				$queryJml = "SELECT * FROM tbwisata";
				$no = $posisi * 1;
			}			
		}
		else {
		...
		$sql="SELECT * FROM tbwisata ORDER BY id_wisata DESC";
		...
		<tr>
			...
			<td><?php echo $r_tampil_anggota['keterangan']; ?></td>
			<td>
					<a href="wisata_edit.php?id=<?php echo $r_tampil_anggota['id_wisata'];?>" class="btn btn-warning">Edit</a>
					<a href="wisata_hapus.php?id=<?php echo $r_tampil_anggota['id_wisata']; ?>" onclick = "return confirm ('Apakah Anda Yakin Akan Menghapus Data Ini?')" class="btn btn-danger">Hapus</a>
			</td>			
		</tr>
		<?php $nomor++; } 
		}
	...
	if($_POST['pencarian']!=''){
		...
		<div class="pagination">		
				...
				for($i=1; $i<=$jml_hal; $i++){
					if($i != $hal){
						echo "<a href=\"?wisata_edit&hal=$i\">$i</a>";
					}
					else {
